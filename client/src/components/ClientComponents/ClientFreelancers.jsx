...
export default function ClientFreelancers() {
  ...
  useEffect(() => {
    tokenExists(token, navigate, dispatch).then(data => {
      ...
      if (!data || userInfo.role !== "client" || userInfo._id !== id) {
        navigate("/login");
      }
    });
  }, []);
  ...
  useEffect(() => {
    dispatch(freelancersServices())
      ...
      .then(data => {
        setTimeout(() => {
          const sortedServices = [...data.allServices];
          sortServices(sortedServices);
  ...
  const sortServices = (services) => {
    if (sort === "price") {
      services.sort((a, b) => (order === "asc" ? a.price - b.price : b.price - a.price));
    } else if (sort === "rating") {
      services.sort((a, b) => (order === "asc" ? a.serviceRating - b.serviceRating : b.serviceRating - a.serviceRating));
    }
  };
